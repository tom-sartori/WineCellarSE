...
//                System.out.println(Facade.getInstance().getReferencingsByCompany(company.getId()).toString());
                listFlowPane.getChildren().add(CardComponent.createCompanyRequestCard(company));
            });
        }catch (NotFoundException e){
            Alert alert = NodeCreations.createAlert("Aucune entreprise à valider", "Il n'y a aucune entreprise à valider pour le moment", "Aucune entreprise à valider", Alert.AlertType.INFORMATION);
            alert.show();
        }
    }
}
